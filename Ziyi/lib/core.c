...
/*
 * Helper function for cmove
 * Make sure the position is in the limit
 */
void mk_within_lim(double *pos, const double increment, const double lim){
	*pos += increment;
...
void cmove(double *heading_rad, double *pos, const double preference,
	...
	mk_within_lim(&pos[0], x, lim);
	mk_within_lim(&pos[1], y, lim);
}


/*
 * the judge function for Simulation.OnlineSim
...
/*
 * Avoid obsession for Simulation.OnlineSim
 */
void avoid_obsession(int *sig_ign, double *preference, int length){
    ...
    for (int i = 0; i < length; i++){
        d = abs(preference[i + 2] - preference[i + 3]);
        d01 = abs(preference[i] - preference[i + 1]) / d;
        d02 = abs(preference[i] - preference[i + 2]) / d;
        d12 = abs(preference[i + 1] - preference[i + 2]) / d;

        // check three points before a significant change (d4 > 1)
        // if the differences between i & i+1, i & i+2, i+1 & i+2 
        // are all small, there should be a "plateau" shape
       	// or if the difference between i & i+2 is small
       	// while the other two are not, there should be a "saw" shape
        if (d > 1){
        	if ((d01 < 0.2 && d02 < 0.2 && d12 < 0.2) ||
        		(d01 > 0.8 && d02 < 0.2 && d12 > 0.8)){
            	occurence ++;
        	}
        }
    }
